...
class RegistroComidasPaciente {
    ...
    getTodayInTimezone(timezone) {
        try {
            ...
            // Formatear la fecha (devuelve formato YYYY-MM-DD)
            const result = formatter.format(now);
            console.log('📅 Fecha calculada en timezone:', timezone, '->', result);
    ...
    async cargarRegistros(periodo = 'semana') {
        try {
            ...
            if (response.ok) {
                ...
                // Si no hay registros, mostrar estado vacío inmediatamente
                if (this.todosLosRegistros.length === 0) {
    ...
    mostrarRegistros() {
        ...
        const registrosHTML = registrosPagina.length > 0 ? 
            registrosPagina
                .sort((a, b) => new Date(b.fecha) - new Date(a.fecha))
                .map(registro => this.crearFilaRegistro(registro))
                .join('') : '';
    ...
    formatearFecha(fecha) {
        try {
            ...
            const fechaObj = new Date(fechaParte + 'T12:00:00'); // Usar mediodía para evitar problemas de zona horaria
            ...
            if (this.profesionalTimezone && this.profesionalTimezone !== 'UTC') {
                ...
                // Si no hay zona horaria específica, usar formato directo
                const [year, month, day] = fechaParte.split('-');
                return `${day}/${month}/${year}`;
            }
        } catch (error) {
            console.warn('Error formateando fecha:', error, 'Fecha:', fecha);
            // Fallback: mostrar la fecha tal como viene de la base de datos
                                    ...
                                    <div class="col-md-6">
                                     ...
                                     <div class="col-md-6">
                                         ...
                                         <input type="date" class="form-control" id="fechaRegistro" 
                                                value="${registro ? registro.fecha : this.getTodayInTimezone(this.profesionalTimezone)}" required>
                                     </div>
                                </div>
                                <div class="mb-3">
    ...
    mostrarEstadoVacio() {
        ...
        container.innerHTML = `
            <div class="mb-3">
                ...
                <div class="row align-items-center">
                    <div class="col-12 col-md-6 mb-2 mb-md-0">
                        ...
                        <span class="text-muted small">0 registros</span>
                    </div>
                    <div class="col-12 col-md-6 text-md-end">
                        <label class="form-label small text-muted mb-1 me-2">Mostrar:</label>
                        <select class="form-select form-select-sm d-inline-block w-auto" id="registrosPorPagina">
            ...
            <div class="text-center p-5">
                ...
                <h4 class="text-muted mb-3">No tienes registros de comidas</h4>
                <p class="text-muted mb-4">Comienza registrando tus comidas diarias para llevar un seguimiento de tu alimentación.</p>
                <button class="btn btn-primary btn-lg" onclick="registroComidasPaciente.mostrarFormularioRegistro()">
                    <i class="fas fa-plus me-2"></i>Registrar mi primera comida
                </button>
            </div>
        `;
